...
describe ActiveTableSet::TableSetConfig do
  context "config" do
    ...
    let(:table_set_cfg) { { :name => "test_ts", :partitions => [partition_cfg]  } }
    let(:config)        { ActiveTableSet::TableSetConfig.new(table_set_cfg) }
    ...
    it "support a dsl for defining the table set" do
      table_set = ActiveTableSet::TableSetConfig.config do |ts|
