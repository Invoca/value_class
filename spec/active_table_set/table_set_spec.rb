...
describe ActiveTableSet::TableSet do
  ...
  context "construction" do
    ...
    it "can construct a table set with multiple partitions" do
      table_set = ActiveTableSet::TableSet.new(multi_table_set_cfg)
    ...
    it "raises an exception if we have more than one partition and any partitions do not have a key" do
      ...
      expect { ActiveTableSet::TableSet.new(cfg) }.to raise_error(ArgumentError, "all partitions must have partition_keys if more than one partition is configured")
    end
  end

  context "partitioned?" do
    it "report false if only a single parititon provided" do
      config = ActiveTableSet::TableSet.new(table_set_cfg)
