    ...
    class Request
      ...
      def access_override_from_process_settings
        ...
        ProcessSettings['active_table_set', scoped_setting_key, 'access_override', required: false]&.to_sym.tap do |current_override|
          ExceptionHandling.logger.debug("ActiveTableSetOverride Check", active_table_set: { table_set: table_set, partition_key: partition_key, override: current_override })
        end
      end
    end
  end
